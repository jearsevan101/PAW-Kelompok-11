...
export default function Dashboard() {
  ...
  const handleDelete = (id) => {
    const userConfirmation = window.confirm("Are you sure you want to delete this record?");
    if (userConfirmation) {
      axios
        ...
        <div className="object-cover relative w-full sm:w-[720px] p-8 h-[screen] rounded-xl overflow-hidden bg-[#FFFFFF] ml-[24px] mb-[36px] mt-[36px]">
          ...
          <div className="object-cover relative w-full sm:w-[720px] p-8 h-[screen] rounded-xl overflow-hidden bg-[#FFFFFF] mb-[36px] mt-[36px]">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <h2 className="font-bold text-c-text-dark text-xl">History Transaction</h2>
            </div>
            {mergedData.map(data => (
                  ...
                  <div>
                    ...
                    <button onClick={() => setSelectedData(data)}
                      style={{
                        ...
                        letterSpacing: '0.05em',
                        fontWeight: 'semi-bold',
                        backgroundColor: data.status === 'DITOLAK' ? '#FFA37A' : data.status === 'KONFIRMASI' ? '#FFF506' : data.status === 'DISEWA' ? '#7EC2FF' : data.status === 'DIAJUKAN' ? '#FFE488' : '#BCE455',
                        color: data.status === 'DITOLAK' ? '#930B16' : data.status === 'KONFIRMASI' ? '#9E7B00' : data.status === 'DISEWA' ? '#1A4393' : data.status === 'DIAJUKAN' ? '#7A4D0B' : '#4C7A0B',
                        display: 'inline-block',
                        padding: '0.3em 2em',
                        border: 'none',
                        borderRadius: '4px',
